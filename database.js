...
const Database = {
    ...
    init: async function () {
        ...
        try {
            // Fetch data from API
            // Note: Ensure the backend server is running on port 3000
            const [hotels, activities, restaurants] = await Promise.all([
                fetch('http://localhost:3000/api/hotels').then(res => res.json()),
                fetch('http://localhost:3000/api/activities').then(res => res.json()),
                fetch('http://localhost:3000/api/restaurants').then(res => res.json())
            ]);
